...
class ItemRemoteDatasource implements IItemRemoteDatasource {
  ...
  Future<String> uploadImage(File image) async {
    // split the file path and only keep the last bit [The actual file name in most cases]
    final fileName = image.path.split('/').last;
    final formData = FormData.fromMap({
      'itemPhoto': await MultipartFile.fromFile(image.path, filename: fileName),
    });
    // get token from token service [{local storage}=>shared preferences/ secure storage]
